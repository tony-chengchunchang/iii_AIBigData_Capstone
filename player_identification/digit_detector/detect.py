...
class DigitSpotter:
    ...
    def run(self, image, threshold=0.7, do_nms=True, show_result=True, nms_threshold=0.3):
        ...
        if len(patches) > 0:
            ...
            index = [a for a in range(len(area_list)) if area_list[a]>=max_area*0.97]
        ...
        if show_result:
            for i, bb in enumerate(bbs):
                ...
                cv2.putText(image, msg, ((x1+x2)//2, (y1+y2)//2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), thickness=1)
